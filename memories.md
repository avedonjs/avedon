...
- **www Product-stage Tailwind (2026-07-29):** Site chrome rebuilt with Tailwind v4 PostCSS (`app.css` `@theme` = create-app tokens); co-located `<style>` removed from www `.ave`; prose/CM/stage/shiki in `app.css`; home no-scroll + mouse grid glow + install in hero. Spec/plan: `docs/superpowers/{specs,plans}/2026-07-29-www-tailwind-redesign*`. Playground iframe unchanged. **Deploying.**
